...
//Currently doesnt really handle TransformType.All
	public class TransformGizmoCustomGizmo : MonoBehaviour
	{
		...
		void Awake()
		{
			leftRay = GameObject.Find("LeftHand Controller").GetComponent<XRRayInteractor>();
			rightRay = GameObject.Find("RightHand Controller").GetComponent<XRRayInteractor>();
		...
		{
			ShowProperGizmoType();
			rightDevice.TryGetFeatureValue(CommonUsages.gripButton, out bool rightTriggerValue);
			if (rightTriggerValue)
			{
				RaycastHit hitInfo;
				rightRay.TryGetHitInfo(out Vector3 reticlePosition, out _, out _, out _);
				if (Physics.Raycast(transformGizmo.myCamera.ScreenPointToRay(reticlePosition), out hitInfo, Mathf.Infinity, mask))
				{
