	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		void TransformSelected()
		{
			if(mainTargetRoot != null)
			{
				...
				if (nearAxis != Axis.None && triggerValue)
				{
		...
		IEnumerator TransformSelected(TransformType transType)
		{
			...
			List<IGizmoCommand> transformCommands = new List<IGizmoCommand>();
			...
			while (triggerValue)
			{
				...
				Vector3 mousePosition = reticlePosition;
				...
				if(previousMousePosition != Vector3.zero && mousePosition != Vector3.zero)
				{
					...
					else if(transType == TransformType.Rotate)
					{
						...
						{
							Vector3 rotation = transform.TransformDirection(new Vector3(reticlePosition.y, -reticlePosition.x, 0));
		...
		void GetTarget()
		{
			...
			if (nearAxis == Axis.None && triggerValue)
			{
				...
				RaycastHit hitInfo;
				rayInteractor.TryGetHitInfo(out Vector3 reticlePosition, out _, out _, out _);
				if (Physics.Raycast(myCamera.ViewportPointToRay(reticlePosition), out hitInfo, Mathf.Infinity, selectionMask)) // DOES THIS WORK?
				{
		...
		void HandleNearest(TransformType type, float xClosestDistance, float yClosestDistance, float zClosestDistance, float allClosestDistance, float minSelectedDistanceCheck)
		{
			...
			{
				rayInteractor.TryGetHitInfo(out Vector3 reticlePosition, out _, out _, out _);
				Ray mouseRay = myCamera.ScreenPointToRay(reticlePosition); // DOES THIS WORK?
		...
		{
			rayInteractor.TryGetHitInfo(out Vector3 reticlePosition, out _, out _, out _);
			Ray mouseRay = myCamera.ScreenPointToRay(reticlePosition);
